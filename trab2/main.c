...
void *produtor(){
    ...
    for(int i=0; i<total/N;i++){ 
        for(int j=0;j<N;j++){
            if(j<4)getdelim(&linha,&buf, 32, file);
        ...
        if((i>0 && i%10==0) || (i == total/N-1)){
            for(int i=0;i<nthreads;i++) sem_post(&scom);
...
void *consumidor(void* t){
    ...
    while(!cabo){
        sem_wait(&scom);
        printf("esperando consumidor %d", *id);
        if(cabo==1) pthread_exit(NULL);
        int aux2=total/N;
        if((aux2%nthreads)!=0) aux2+=(aux2%nthreads);
        ...
        for(int i=0;i<aux2;i++){
            for (int k = 1; k < N; k++) {
                for (int j = 0; j < N - 1; j++) {
                    ...
                    if (buffer[inicio] > buffer[inicio + 1]) {
